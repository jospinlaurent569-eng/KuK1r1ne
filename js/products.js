...
function productCardHTML(p, listView = false) {
  ...
  <article class="p-card" data-id="${p.id}">
    <div class="p-media">
      ${discount > 0 ? `<span class="p-badge">-${discount}%</span>` : ''}
    ...
    <div class="p-body">
      ...
      <div class="p-price">${discount > 0 ? `<span class="price-old">${formatPrice(p.prix_original)}</span>` : ''}<span class="price-new">${formatPrice(p.prix_remise)}</span></div>
