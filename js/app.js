  ...
  el.innerHTML = `
    <div class="container header-inner">
      ...
      <div class="header-actions">
        ...
        <a class="icon-btn" href="wishlist.html" aria-label="Favoris">${icon('heart')}<span class="badge-count wishlist-badge" style="display:none">0</span></a>
        <button class="icon-btn cart-toggle" aria-label="Panier">${icon('cart')}<span class="badge-count cart-badge" style="display:none">0</span></button>
      </div>
...
function renderHomeContent() {
  ...
  document.getElementById('hero-cta-shop').textContent = h.ctaShop;
    ...
    <div class="pd-grid">
      ...
      <div class="pd-info">
        ...
        <div class="pd-price-row">
          ${discount > 0 ? `<span class="price-old" style="font-size:16px">${formatPrice(p.prix_original)}</span>` : ''}
          <span class="price-new">${formatPrice(p.prix_remise)}</span>
          ${discount > 0 ? `<span class="p-badge" style="position:static">-${discount}%</span>` : ''}
        </div>
        <div class="stock-row ${p.stock < 6 ? 'low' : ''}">${icon(p.stock < 6 ? 'alert' : 'check')} ${p.stock < 6 ? pr.lowStock(p.stock) : pr.stockOk(p.stock)}</div>
